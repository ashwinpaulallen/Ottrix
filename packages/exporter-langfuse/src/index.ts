...
import type { SpanData, TraceData, TraceExporter } from 'ottrix';
...
/** Langfuse public ingestion batch event. */
...
/** Exports ottrix {@link TraceData} to Langfuse via the public ingestion API. */
export class LangfuseExporter implements TraceExporter {
  ...
  constructor(options: LangfuseExporterOptions) {
    ...
    this.flushIntervalMs = options.flushInterval ?? options.flushIntervalMs ?? 5_000;
  ...
  private async sendBatch(events: LangfuseBatchEvent[]): Promise<boolean> {
    ...
    try {
      const response = await this.fetchImpl(`${this.baseUrl}/api/public/ingestion`, {
        method: 'POST',
        headers: {
      ...
      if (response.status === 401 || response.status === 403) {
        logExporterError(this.name, 'Authentication failed — check Langfuse public/secret keys');
        return true;
      }

      if (response.status === 429) {
        logExporterError(this.name, 'Rate limited by Langfuse');
        return false;
      }
      ...
      logExporterError(this.name, 'Network error while sending batch', error);
      return false;
    }
  }
}
...
function createGenerationEvent(traceId: string, span: SpanData): LangfuseBatchEvent {
  ...
  const costUsd = readNumber(span.attributes['llm.cost_usd'] ?? span.attributes.costUsd);
...
function logExporterError(exporter: string, message: string, error?: unknown): void {
  const detail =
