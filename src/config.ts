...
/** Log level for agent-kit (includes `silent` to disable logging). */
 ...
 *
 * @deprecated Prefer `AgenticProviderConfig` — `ProviderConfig` in `agent-kit/types` is the LLM connection type.
 */
...
/**
 * Fully resolved agent-kit configuration.
 */
...
export interface LoadConfigOptions {
  /** Project root used to discover `.agentkitrc.*` / `.agenticrc.*` files. @defaultValue `process.cwd()` */
  cwd?: string;
...
/**
 * Type-safe helper for `.agentkitrc` / `.agenticrc` and `agent.config` modules (Vite-style).
 *
 * @example
 * ```ts
 * // agentic.config.ts
 * import { defineConfig } from 'agent-kit';
 ...
 *
 * Priority (highest first): `overrides` → environment (`AGENT_KIT_*`, legacy `AGENTIC_*`) → config file → defaults.
 */
export function loadConfig(options: LoadConfigOptions = {}): LoadConfigResult {
  ...
  if (issues.length > 0) {
    throw new ConfigValidationError(
      `Invalid agent-kit configuration: ${issues.map((i) => i.message).join('; ')}`,
      issues,
    );
  }
...
/**
 * Discover config file: `AGENT_KIT_CONFIG_PATH` (or legacy `AGENTIC_CONFIG_PATH`),
 * then `.agentkitrc.*`, then `.agenticrc.*`.
 */
export function discoverConfigFile(
  ...
  const explicit =
    readEnv(env, 'AGENT_KIT_CONFIG_PATH') ?? readEnv(env, 'AGENTIC_CONFIG_PATH');
...
/** Parse `AGENT_KIT_*` (and legacy `AGENTIC_*`) env vars into a partial config. */
export function readConfigFromEnv(env: NodeJS.ProcessEnv = process.env): AgenticConfigInput {
  ...
  const defaultProvider =
    readKitEnv(env, 'DEFAULT_PROVIDER') ?? readKitEnv(env, 'PROVIDER');
  if (defaultProvider) {
    partial.defaultProvider = defaultProvider;
  }

  const defaultModel = readKitEnv(env, 'DEFAULT_MODEL') ?? readKitEnv(env, 'MODEL');
  if (defaultModel) {
    partial.defaultModel = defaultModel;
  }

  const maxSteps = parsePositiveInt(readKitEnv(env, 'MAX_STEPS'));
  if (maxSteps !== undefined) {
    partial.maxSteps = maxSteps;
  }

  const maxTokenBudget = parsePositiveInt(readKitEnv(env, 'MAX_TOKEN_BUDGET'));
  if (maxTokenBudget !== undefined) {
    partial.maxTokenBudget = maxTokenBudget;
  }

  const logLevel = parseLogLevel(readKitEnv(env, 'LOG_LEVEL'));
  if (logLevel) {
    partial.logLevel = logLevel;
  }

  const telemetryEnabled = parseBoolean(readKitEnv(env, 'TELEMETRY_ENABLED'));
  const telemetryExporter = readKitEnv(env, 'TELEMETRY_EXPORTER');
  ...
  const piiDetection = parseBoolean(readKitEnv(env, 'GUARDRAILS_PII_DETECTION'));
  const maxCostUsd = parsePositiveFloat(readKitEnv(env, 'GUARDRAILS_MAX_COST_USD'));
  ...
  for (const provider of ['anthropic', 'openai', 'ollama'] as const) {
    ...
    const apiKey = readKitEnv(env, `${upper}_API_KEY`);
    const baseUrl = readKitEnv(env, `${upper}_BASE_URL`);
    const model = readKitEnv(env, `${upper}_MODEL`);
...
function collectEnvWarnings(env: NodeJS.ProcessEnv, warnings: ConfigWarning[]): void {
  const logLevelRaw = readKitEnv(env, 'LOG_LEVEL');
  if (logLevelRaw && !parseLogLevel(logLevelRaw)) {
    warnings.push({
      code: 'invalid_env_value',
      message: `AGENT_KIT_LOG_LEVEL (or AGENTIC_LOG_LEVEL) "${logLevelRaw}" is invalid; expected debug, info, warn, error, or silent`,
      path: 'AGENT_KIT_LOG_LEVEL',
    });
  }

  const maxStepsRaw = readKitEnv(env, 'MAX_STEPS');
  if (maxStepsRaw && parsePositiveInt(maxStepsRaw) === undefined) {
    warnings.push({
      code: 'invalid_env_value',
      message: `AGENT_KIT_MAX_STEPS (or AGENTIC_MAX_STEPS) "${maxStepsRaw}" is not a positive integer`,
      path: 'AGENT_KIT_MAX_STEPS',
    });
  }
}
...
/** Read `AGENT_KIT_*` with fallback to legacy `AGENTIC_*` environment variables. */
function readKitEnv(env: NodeJS.ProcessEnv, suffix: string): string | undefined {
  return readEnv(env, `AGENT_KIT_${suffix}`) ?? readEnv(env, `AGENTIC_${suffix}`);
}
