...
export interface McpSseServerTransportOptions {
  /** TCP port. @defaultValue 3001 */
  port?: number;
  /** Bind host. @defaultValue '127.0.0.1' */
  host?: string;
  /** SSE path. @defaultValue '/sse' */
  ssePath?: string;
  /** JSON-RPC POST path. @defaultValue '/message' */
  messagePath?: string;
  /** Keepalive interval in milliseconds. @defaultValue 30000 */
  keepaliveMs?: number;
  /** Maximum JSON-RPC POST body size in bytes. @defaultValue 1048576 */
  maxBodyBytes?: number;
}
...
export class McpSseServerTransport implements MCPServerTransport {
  ...
  /**
   * @param options - Port, paths, and keepalive settings.
   */
  ...
  /** Resolved base URL after {@link start}. */
  getBaseUrl(): string {
  ...
  /** Send a JSON-RPC response to a specific SSE client. */
