...
import { OTTRIX_INJECTION_GUARD_OPTIONS, OTTRIX_RUN_CONTEXT_OPTIONS } from '../tokens.js';
...
export function resolveHttpOptions(http?: OttrixHttpOptions): ResolvedOttrixHttpOptions {
  if (http === false) {
    return { runContext: false, telemetry: false, injectionGuard: false };
  }

  if (http === true) {
    return { runContext: true, telemetry: true, injectionGuard: true };
  }
