...
describe('readConfigFromEnv', () => {
  it('parses OTTRIX_* variables', () => {
    const partial = readConfigFromEnv({
      OTTRIX_PROVIDER: 'openai',
      OTTRIX_MODEL: 'gpt-4o-mini',
      OTTRIX_MAX_STEPS: '12',
    });
  ...
  it('prefers OTTRIX_* over AGENT_KIT_* and AGENTIC_*', () => {
    const partial = readConfigFromEnv({
      OTTRIX_PROVIDER: 'ollama',
      AGENT_KIT_PROVIDER: 'openai',
      AGENTIC_PROVIDER: 'anthropic',
    });

    expect(partial.defaultProvider).toBe('ollama');
  });
...
`,
  ...
  it('discovers .ottrixrc.json before legacy config files', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'ottrix-config-'));
    writeFileSync(join(tempDir, '.agenticrc.json'), JSON.stringify({ maxSteps: 3 }));
    writeFileSync(join(tempDir, '.agentkitrc.json'), JSON.stringify({ maxSteps: 5 }));
    writeFileSync(join(tempDir, '.ottrixrc.json'), JSON.stringify({ maxSteps: 7 }));

    expect(discoverConfigFile(tempDir)).toBe(join(tempDir, '.ottrixrc.json'));
  });
