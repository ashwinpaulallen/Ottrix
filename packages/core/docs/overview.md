...
| `ottrix/exporters/*` | `dist/observability/exporters/*.js` | Built-in webhook exporter |
| `@ottrix/mcp-server` | *(separate npm package)* | MCP server hosting, `ottrix-serve` CLI |
| `@ottrix/exporter-otel` | *(separate npm package)* | OTLP/HTTP OpenTelemetry exporter |
| `@ottrix/exporter-langfuse` | *(separate npm package)* | Langfuse trace export |
| `@ottrix/exporter-braintrust` | *(separate npm package)* | Braintrust trace export |
| `@ottrix/nestjs` | *(separate npm package)* | NestJS DI adapter — install alongside `ottrix` |

CLI: **`npx ottrix-serve`** from `@ottrix/mcp-server` (no longer bundled in `ottrix`).
...
| Trace exporters | Core: `WebhookExporter`, `MultiExporter` — standalone: `@ottrix/exporter-*` packages |
...
| `a40ea26` | `@ottrix/nestjs` adapter | [NestJS docs](../../nestjs/docs/guide.md) |
