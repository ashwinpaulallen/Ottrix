...
/** Log level for Ottrix (includes `silent` to disable logging). */
 ...
 *
 * @deprecated Prefer `AgenticProviderConfig` — `ProviderConfig` in `ottrix/types` is the LLM connection type.
 */
...
/**
 * Fully resolved Ottrix configuration.
 */
...
export interface LoadConfigOptions {
  /** Project root used to discover `.ottrixrc.*` / legacy config files. @defaultValue `process.cwd()` */
  cwd?: string;
...
/**
 * Type-safe helper for `.ottrixrc` / legacy config files and `agent.config` modules (Vite-style).
 *
 * @example
 * ```ts
 * // agentic.config.ts
 * import { defineConfig } from 'ottrix';
 ...
 *
 * Priority (highest first): `overrides` → environment (`OTTRIX_*`, legacy `AGENT_KIT_*` / `AGENTIC_*`) → config file → defaults.
 */
export function loadConfig(options: LoadConfigOptions = {}): LoadConfigResult {
  ...
  if (issues.length > 0) {
    throw new ConfigValidationError(
      `Invalid Ottrix configuration: ${issues.map((i) => i.message).join('; ')}`,
      issues,
    );
  }
...
/**
 * Discover config file: `OTTRIX_CONFIG_PATH` (or legacy `AGENT_KIT_CONFIG_PATH` / `AGENTIC_CONFIG_PATH`),
 * then `.ottrixrc.*`, then legacy `.agentkitrc.*` / `.agenticrc.*`.
 */
export function discoverConfigFile(
  ...
  const explicit =
    readEnv(env, 'OTTRIX_CONFIG_PATH') ??
    readEnv(env, 'AGENT_KIT_CONFIG_PATH') ??
    readEnv(env, 'AGENTIC_CONFIG_PATH');
...
/** Parse `OTTRIX_*` (and legacy `AGENT_KIT_*` / `AGENTIC_*`) env vars into a partial config. */
export function readConfigFromEnv(env: NodeJS.ProcessEnv = process.env): AgenticConfigInput {
  ...
  const defaultProvider =
    readOttrixEnv(env, 'DEFAULT_PROVIDER') ?? readOttrixEnv(env, 'PROVIDER');
  if (defaultProvider) {
    partial.defaultProvider = defaultProvider;
  }

  const defaultModel = readOttrixEnv(env, 'DEFAULT_MODEL') ?? readOttrixEnv(env, 'MODEL');
  if (defaultModel) {
    partial.defaultModel = defaultModel;
  }

  const maxSteps = parsePositiveInt(readOttrixEnv(env, 'MAX_STEPS'));
  if (maxSteps !== undefined) {
    partial.maxSteps = maxSteps;
  }

  const maxTokenBudget = parsePositiveInt(readOttrixEnv(env, 'MAX_TOKEN_BUDGET'));
  if (maxTokenBudget !== undefined) {
    partial.maxTokenBudget = maxTokenBudget;
  }

  const logLevel = parseLogLevel(readOttrixEnv(env, 'LOG_LEVEL'));
  if (logLevel) {
    partial.logLevel = logLevel;
  }

  const telemetryEnabled = parseBoolean(readOttrixEnv(env, 'TELEMETRY_ENABLED'));
  const telemetryExporter = readOttrixEnv(env, 'TELEMETRY_EXPORTER');
  ...
  const piiDetection = parseBoolean(readOttrixEnv(env, 'GUARDRAILS_PII_DETECTION'));
  const maxCostUsd = parsePositiveFloat(readOttrixEnv(env, 'GUARDRAILS_MAX_COST_USD'));
  ...
  for (const provider of ['anthropic', 'openai', 'ollama'] as const) {
    ...
    const apiKey = readOttrixEnv(env, `${upper}_API_KEY`);
    const baseUrl = readOttrixEnv(env, `${upper}_BASE_URL`);
    const model = readOttrixEnv(env, `${upper}_MODEL`);
...
function collectEnvWarnings(env: NodeJS.ProcessEnv, warnings: ConfigWarning[]): void {
  const logLevelRaw = readOttrixEnv(env, 'LOG_LEVEL');
  if (logLevelRaw && !parseLogLevel(logLevelRaw)) {
    warnings.push({
      code: 'invalid_env_value',
      message: `OTTRIX_LOG_LEVEL (or AGENT_KIT_LOG_LEVEL / AGENTIC_LOG_LEVEL) "${logLevelRaw}" is invalid; expected debug, info, warn, error, or silent`,
      path: 'OTTRIX_LOG_LEVEL',
    });
  }

  const maxStepsRaw = readOttrixEnv(env, 'MAX_STEPS');
  if (maxStepsRaw && parsePositiveInt(maxStepsRaw) === undefined) {
    warnings.push({
      code: 'invalid_env_value',
      message: `OTTRIX_MAX_STEPS (or AGENT_KIT_MAX_STEPS / AGENTIC_MAX_STEPS) "${maxStepsRaw}" is not a positive integer`,
      path: 'OTTRIX_MAX_STEPS',
    });
  }
}
...
/** Read `OTTRIX_*` with fallback to legacy `AGENT_KIT_*` and `AGENTIC_*` environment variables. */
function readOttrixEnv(env: NodeJS.ProcessEnv, suffix: string): string | undefined {
  return (
    readEnv(env, `OTTRIX_${suffix}`) ??
    readEnv(env, `AGENT_KIT_${suffix}`) ??
    readEnv(env, `AGENTIC_${suffix}`)
  );
}
