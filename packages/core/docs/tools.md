...
| Default | Value |
|---------|-------|
| `clientInfo` | `{ name: 'ottrix', version: '1.0.0' }` |
| `requestTimeoutMs` | `30_000` |
...
`validateSchema` exported from `ottrix/tools` (implemented in `src/utils/schema-validator.ts`).
...
## MCP server (`@ottrix/mcp-server`)

Install: `npm install @ottrix/mcp-server ottrix`
...
**CLI:** `npx ottrix-serve --transport stdio` (from `@ottrix/mcp-server`)

**Package:** `@ottrix/mcp-server` — MCP server hosting; MCP **client** remains in `ottrix/tools`.
...
**MCP server:** install `@ottrix/mcp-server` for `MCPServer`, `serveMCP`, `ASK_AGENT_TOOL_NAME`, and the `ottrix-serve` CLI.
